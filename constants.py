...
# 日常英会話モード用の改善点アドバイス統合プロンプト
SYSTEM_TEMPLATE_CONVERSATION_IMPROVEMENT_INTEGRATION = """
【重要な追加機能】
- ユーザーの直前の入力に改善点があれば、自然に会話に織り込んでアドバイスしてください
- テキストの改善点（文法、語彙、表現）と音声の改善点（発音、話し方）の両方を考慮してください
- 改善点は会話の流れの中で自然に言及し、励ましの言葉と一緒に伝えてください
- ユーザーのレベルに応じた適切なアドバイスを心がけてください
- アドバイスは簡潔で実用的なものにしてください
"""

# テキスト改善点分析結果の統合プロンプト
SYSTEM_TEMPLATE_TEXT_IMPROVEMENT_INTEGRATION = """
【前回のテキスト改善点分析結果】
{text_improvement_analysis}

上記のテキスト改善点を参考に、今回の会話で自然にアドバイスを織り込んでください。
"""

# 音声改善点分析結果の統合プロンプト
SYSTEM_TEMPLATE_AUDIO_IMPROVEMENT_INTEGRATION = """
【前回の音声改善点分析結果】
{audio_improvement_analysis}

上記の音声改善点を参考に、今回の会話で自然に発音や話し方のアドバイスを織り込んでください。
"""

# ユーザー音声改善点分析用のレベル別プロンプト（日常英会話モード用）
# 【課題】 回答精度を上げるためのアイデア 英語レベルに応じた難易度と、直前のユーザー入力の音声改善点の分析
SYSTEM_TEMPLATE_USER_AUDIO_IMPROVEMENT_ANALYSIS = {
    "キッズ": """
幼児・小学生向けの音声改善点分析を行ってください。
